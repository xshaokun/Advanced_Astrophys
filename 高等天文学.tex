...
\subsection{等照面}
描绘相同表面亮度的轮廓被称为\textbf{等照面}，天文学家一直希望通过定义某个等照面作为星系的半径。对于银河系，一种常用的定义方式是规定具有表面亮度$\mu_H=26.5 B-\mathrm{mag\;arcsec^{-2}}$等照面的椭球的投影半长轴为半径，这被称为\textbf{霍姆伯格半径（Holmberg radius）}$r_H$。还有一种常用半径是\textbf{有效半径}$r_e$，其定义是包含了星系一半光度的投影半径。

在有效半径$r_e$处的表面亮度$\mu_e$的大小，与星系的表面亮度随半径的分布有关，而这个分布为\textbf{Sérsic profile}
\begin{equation}
  \mu(r)=\mu_e+8.3268\left[\left({r\over r_e}\right)^{1/n}-1\right]
\end{equation}

它是银核的$r^{1/4}$ de Vaucouleurs profile的推广形式，其中$n, \mu_e, \r_e$是用于与实际表面亮度拟合的自由参数。
